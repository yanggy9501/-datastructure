...
public class GraphHelper {
     ...
     * @return Graph图
     */
    public static <T> MatrixGraph<T> createGraph(int[][] edges, List<T> vertexes) {
        MatrixGraph<T> matrixGraph = new MatrixGraph<>(vertexes.size());
        // add vertex
        ...
        for (int i = 0; i < edges.length; i++) {
            ...
            for (int j = 0; j < vertexEdge.length; j++) {
                matrixGraph.addEdge(i, j, edges[i][j]);
            }
        }
        return matrixGraph;
    }
     ...
     * @return 最小生成树
     */
    public static <T> MatrixGraph<T> findMstByPrim(MatrixGraph<T> matrixGraph, int begin) {
        ...
        for (int i = 0; i < vertexNumber - 1; i++) {
            ...
            for (int v = 0; v < vertexNumber; v++) {
                if (!visited[v] && matrixGraph.getWeight(v, index) < weight[v]) {
                    parent[v] = index;
                    weight[v] = matrixGraph.getWeight(v, index);
                }
            }
        }
     ...
     * @return 最小生成树
     */
    private static <T> MatrixGraph<T> createMstTreeByPrim(int[] parent, MatrixGraph<T> matrixGraph) {
        MatrixGraph<T> mstMatrixGraph = new MatrixGraph<>(matrixGraph.getVertexTotal());
        for (int i = 0; i < parent.length; i++) {
